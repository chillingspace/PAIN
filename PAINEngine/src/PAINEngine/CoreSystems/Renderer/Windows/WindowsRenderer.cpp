...
WindowsRenderer::WindowsRenderer() { PN_CORE_INFO("WindowsRenderer ctor"); }

WindowsRenderer::~WindowsRenderer() { Cleanup(); }

void WindowsRenderer::initSceneVbo(
    const std::vector<PAIN::ModelRenderer> &models) {
  if (!geometry_vbo)
  ...
  for (const auto &mdl : models) {
    if (!mdl.cachedModelAsset ||
        referenced.find(mdl.cachedModelAsset->vpath) != referenced.end()) {
      continue;
    }
    ...
    indices.insert(indices.end(), translated_indices.begin(),
                   translated_indices.end());
  }

  glBindBuffer(GL_ARRAY_BUFFER, geometry_vbo);
  glBufferData(GL_ARRAY_BUFFER, total_vertices * sizeof(Assets::Vertex),
               vertices.data(), GL_STATIC_DRAW);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, geometry_ebo);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, total_indices * sizeof(unsigned int),
  ...
  glBufferData(GL_ARRAY_BUFFER, referenced.size() * sizeof(IBOData), nullptr,
               GL_DYNAMIC_DRAW);
...
void WindowsRenderer::DrawShadows(const ModelRenderer &component,
  ...
  glBufferSubData(GL_ARRAY_BUFFER, 0,
                  component.cachedModelAsset->vertices.size() *
                      sizeof(Assets::Vertex),
                  component.cachedModelAsset->vertices.data());
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, shadow_ebo);
  glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0,
                  component.cachedModelAsset->indices.size() *
                      sizeof(unsigned int),
                  component.cachedModelAsset->indices.data());

  if (component.cachedModelAsset->submeshes.empty()) {
    // No submeshes - draw entire model
    glDrawElements(GL_TRIANGLES, component.cachedModelAsset->indices.size(),
                   GL_UNSIGNED_INT, 0);
  } else {
...
void WindowsRenderer::DebugPass(const glm::vec3 &min_p, const glm::vec3 &max_p,
  ...
  glm::vec3 v[8] = {{min_p.x, min_p.y, min_p.z}, {max_p.x, min_p.y, min_p.z},
...
void WindowsRenderer::DebugPassOBB(const glm::vec3 corners[8],
  ...
  // Edge index list (same as AABB: 12 edges connecting 8 corners)
  // Corners expected in order:
