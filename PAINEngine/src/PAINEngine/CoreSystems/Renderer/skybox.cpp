...
namespace PAIN {
	...
	void Skybox::generatePrefilterMap() {
		...
		glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
		glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
		// GL_LINEAR_MIPMAP_NEAREST: PBR shader uses explicit textureLod() with fractional LOD
		// for roughness interpolation. Using LINEAR between mips causes over-blending on some
		// Android GPU drivers, producing excessively bright specular reflections.
