...
void System::syncNewBodies(entt::registry &registry) {
  ...
  for (auto &&[entity, rigidBody, transform] : view.each()) {
    ...
    if (rigidBody.bodyID.IsInvalid()) {
      ...
      // Check if entity has a compound collider with custom shapes
      if (registry.all_of<CompoundCollider>(entity)) {
        auto &cc = registry.get<CompoundCollider>(entity);
        if (cc.useCompoundCollider && !cc.shapes.empty()) {
          // Build compound shape from multiple sub-shapes
          JPH::StaticCompoundShapeSettings compoundSettings;

          for (const auto &shape : cc.shapes) {
            // Calculate offset with transform scale applied
            JPH::Vec3 shapeOffset(shape.offset.x * transform.scale.x,
                                  shape.offset.y * transform.scale.y,
                                  shape.offset.z * transform.scale.z);

            // Convert rotation
            JPH::Quat shapeRot(shape.rotation.x, shape.rotation.y,
                               shape.rotation.z, shape.rotation.w);

            // Create sub-shape based on type
            JPH::Ref<JPH::Shape> subShape;
            switch (shape.type) {
            case ColliderShapeType::Box:
              subShape = new JPH::BoxShape(
                  JPH::Vec3(shape.boxHalfExtents.x * transform.scale.x *
                                rigidBody.collider_scale.x,
                            shape.boxHalfExtents.y * transform.scale.y *
                                rigidBody.collider_scale.y,
                            shape.boxHalfExtents.z * transform.scale.z *
                                rigidBody.collider_scale.z),
                  0.0f);
              break;
            case ColliderShapeType::Sphere: {
              // Use max scale dimension for sphere radius
              float maxScale = glm::max(
                  transform.scale.x * rigidBody.collider_scale.x,
                  glm::max(transform.scale.y * rigidBody.collider_scale.y,
                           transform.scale.z * rigidBody.collider_scale.z));
              subShape = new JPH::SphereShape(shape.sphereRadius * maxScale);
            } break;
            case ColliderShapeType::Capsule: {
              // Capsule: height along Y axis, radius in XZ
              float heightScale =
                  transform.scale.y * rigidBody.collider_scale.y;
              float radiusScale =
                  glm::max(transform.scale.x * rigidBody.collider_scale.x,
                           transform.scale.z * rigidBody.collider_scale.z);
              subShape =
                  new JPH::CapsuleShape(shape.capsuleHalfHeight * heightScale,
                                        shape.capsuleRadius * radiusScale);
            } break;
            }

            if (subShape) {
              compoundSettings.AddShape(shapeOffset, shapeRot, subShape);
            }
          }
          ...
          if (result.IsValid()) {
            ...
            PN_CORE_TRACE(
                "Created compound collider with {} shapes for entity {}",
                cc.shapes.size(), (uint32_t)entity);
          }
        }
      }

      // Fallback: create single box shape if no compound collider
      if (!finalShape) {
