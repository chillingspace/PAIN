...
// Forward declarations for types used as pointers/references within the PAIN
// namespace
namespace PAIN {
namespace Assets {
class Model;
...
public:
  ...
  queryAABB(const AABB &queryBox, entt::registry &registry,
...
private:
  ...
  // Helper to compute AABB from model vertices (fallback)
  AABB calculateLocalAABB(const std::shared_ptr<Assets::Model> &model);

  // Helper to compute AABB from physics collider (prioritizes CompoundCollider,
  // then RigidBody3D)
  AABB calculateColliderAABB(
      const Physics::RigidBody3D &rb,
      const CompoundCollider *compCollider, // nullptr if not present
      const WorldTransform &transform, const glm::vec3 &entityScale);

  // Check if collision should happen based on layers
  bool shouldCollide(entt::entity entityA, entt::entity entityB);

  bool rayAABBIntersect(const glm::vec3 &origin, const glm::vec3 &direction,
  ...
  detectCollisions(entt::registry &registry);
};
