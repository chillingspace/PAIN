...
AABB sBVHSystem::calculateLocalAABB(
  ...
  if (extents.x < minExtent) {
    localAABB.min.x -= minExtent;
    localAABB.max.x += minExtent;
  }
  if (extents.y < minExtent) {
    localAABB.min.y -= minExtent;
    localAABB.max.y += minExtent;
  }
  if (extents.z < minExtent) {
    localAABB.min.z -= minExtent;
    localAABB.max.z += minExtent;
  }

  return localAABB;
}

// Calculate AABB from physics collider (prioritizes CompoundCollider, then
// RigidBody3D) Uses OBB computation for rotated entities to get enclosing
// world-space AABB
AABB sBVHSystem::calculateColliderAABB(const Physics::RigidBody3D &rb,
                                       const CompoundCollider *compCollider,
                                       const WorldTransform &transform,
                                       const glm::vec3 &entityScale) {

  AABB worldAABB;
  worldAABB.min = glm::vec3(std::numeric_limits<float>::max());
  worldAABB.max = glm::vec3(std::numeric_limits<float>::lowest());

  // Extract rotation from transform matrix to check if rotated
  glm::mat3 rotationMat = glm::mat3(transform.matrix);
  glm::vec3 worldPos = glm::vec3(transform.matrix[3]);

  // Helper lambda to expand worldAABB with a local box (handles OBB if rotated)
  auto expandWithLocalBox = [&](const glm::vec3 &localCenter,
                                const glm::vec3 &halfExtents) {
    // Apply entity scale to half extents
    glm::vec3 scaledHalfExtents = halfExtents * entityScale;

    // Check if entity has rotation (non-identity rotation matrix)
    bool hasRotation =
        glm::length(rotationMat[0] - glm::vec3(1, 0, 0)) > 0.001f ||
        glm::length(rotationMat[1] - glm::vec3(0, 1, 0)) > 0.001f ||
        glm::length(rotationMat[2] - glm::vec3(0, 0, 1)) > 0.001f;

    if (hasRotation) {
      // OBB: Compute 8 corners and find enclosing AABB
      glm::vec3 corners[8] = {
          localCenter + glm::vec3(-scaledHalfExtents.x, -scaledHalfExtents.y,
                                  -scaledHalfExtents.z),
          localCenter + glm::vec3(scaledHalfExtents.x, -scaledHalfExtents.y,
                                  -scaledHalfExtents.z),
          localCenter + glm::vec3(-scaledHalfExtents.x, scaledHalfExtents.y,
      ...
      for (int i = 0; i < 8; ++i) {
        ...
        glm::vec3 worldCorner = rotationMat * corners[i] + worldPos;
        worldAABB.min = glm::min(worldAABB.min, worldCorner);
        worldAABB.max = glm::max(worldAABB.max, worldCorner);
      }
    } else {
      // AABB: No rotation, just translate
      glm::vec3 worldCenter = localCenter * entityScale + worldPos;
      worldAABB.min = glm::min(worldAABB.min, worldCenter - scaledHalfExtents);
      worldAABB.max = glm::max(worldAABB.max, worldCenter + scaledHalfExtents);
    }
  };

  // Check for CompoundCollider first
  if (compCollider && compCollider->useCompoundCollider &&
    ...
    for (const auto &shape : compCollider->shapes) {
      ...
      case ColliderShapeType::Sphere:
        halfExtents = glm::vec3(shape.sphereRadius);
        break;
      case ColliderShapeType::Capsule:
        // Capsule: radius on X/Z, halfHeight + radius on Y
        halfExtents = glm::vec3(shape.capsuleRadius,
                                shape.capsuleHalfHeight + shape.capsuleRadius,
                                shape.capsuleRadius);
        break;
      }

      expandWithLocalBox(shape.offset, halfExtents);
    }
  } else {
    // Use RigidBody3D collider_scale and collider_offset
    glm::vec3 halfExtents = glm::vec3(0.5f) * rb.collider_scale;
    expandWithLocalBox(rb.collider_offset, halfExtents);
  }

  // Ensure valid AABB (in case no shapes were processed)
  if (worldAABB.min.x > worldAABB.max.x) {
    worldAABB.min = worldPos - glm::vec3(0.5f);
    worldAABB.max = worldPos + glm::vec3(0.5f);
  }
...
std::vector<entt::entity> sBVHSystem::queryAABB(const AABB &queryBox,
    ...
    if (node.isLeaf()) {
      ...
      if (queryLayerID != -1) {
        ...
        if (entityLayer) {
          ...
          if (!scnManager->canLayersInteract(queryLayerID,
                                             entityLayer->layer_id)) {
...
bool sBVHSystem::shouldCollide(entt::entity entityA, entt::entity entityB) {
...
std::optional<sBVHSystem::RaycastHit>
sBVHSystem::raycast(const glm::vec3 &origin, const glm::vec3 &direction,
                    float maxDistance, entt::registry &registry,
                    int layerMask) {
  if (!m_bvh.isBuilt()) {
    return std::nullopt;
  }
    ...
    if (node.isLeaf()) {
      ...
      if (rayAABBIntersect(origin, rayDir, bv->worldAABB, tMin, tMax)) {
        if (tMin >= 0 && tMin < closestHit.distance) {
          ...
          closestHit.normal =
              calculateAABBNormal(bv->worldAABB, closestHit.point);
...
bool sBVHSystem::rayAABBIntersect(const glm::vec3 &origin,
                                  const glm::vec3 &direction, const AABB &aabb,
...
glm::vec3 sBVHSystem::calculateAABBNormal(const AABB &aabb,
                                          const glm::vec3 &point) {
  const float epsilon = 0.001f;

  // Check which face was hit
  if (std::abs(point.x - aabb.min.x) < epsilon)
    return glm::vec3(-1, 0, 0);
  if (std::abs(point.x - aabb.max.x) < epsilon)
    return glm::vec3(1, 0, 0);
  if (std::abs(point.y - aabb.min.y) < epsilon)
    return glm::vec3(0, -1, 0);
  if (std::abs(point.y - aabb.max.y) < epsilon)
    return glm::vec3(0, 1, 0);
  if (std::abs(point.z - aabb.min.z) < epsilon)
    return glm::vec3(0, 0, -1);
  if (std::abs(point.z - aabb.max.z) < epsilon)
    return glm::vec3(0, 0, 1);
...
sBVHSystem::detectCollisions(entt::registry &registry) {
...
void sBVHSystem::onUpdate(AppTiming timing, entt::registry &registry) {
  ...
  for (auto [entity, bvComponent, transform] : bvGroup.each()) {
    ...
    if (bvComponent.needsUpdate) {
      ...
      // Check if entity has RigidBody3D - use collider-based AABB
      auto *rb = registry.try_get<Physics::RigidBody3D>(entity);
      if (rb) {
        auto *compCollider = registry.try_get<CompoundCollider>(entity);
        auto *localTrans = registry.try_get<LocalTransform>(entity);
        glm::vec3 entityScale =
            localTrans ? localTrans->scale : glm::vec3(1.0f);
        newWorldAABB =
            calculateColliderAABB(*rb, compCollider, transform, entityScale);
      } else {
        // Fallback to model-based AABB transformation
        newWorldAABB = bvComponent.localAABB.transform(transform.matrix);
      }
      ...
      if (!(bvComponent.worldAABB == newWorldAABB)) {
        ...
        if (m_bvh.isBuilt() && !m_needsFullRebuild &&
  ...
  auto needsBVView = registry.view<ModelRenderer, WorldTransform>(
      entt::exclude<BoundingVolume>);

  for (auto [entity, modelRenderer, transform] : needsBVView.each()) {

    // Early validation (Fix #6)
    if (!modelRenderer.modelGUID.IsValid())
      continue;

    auto model_opt =
        getServices()->get<Assets::Manager>()->getAsset<Assets::Model>(
            modelRenderer.modelGUID);
    if (!model_opt.has_value() || model_opt.value()->vertices.empty())
      continue;
    ...
    if (rb) {
      auto *compCollider = registry.try_get<CompoundCollider>(entity);
      auto *localTrans = registry.try_get<LocalTransform>(entity);
      glm::vec3 entityScale = localTrans ? localTrans->scale : glm::vec3(1.0f);
      // For collider-based entities, localAABB is just a unit box (unused)
      bvComponent.localAABB.min = glm::vec3(-0.5f);
      bvComponent.localAABB.max = glm::vec3(0.5f);
      bvComponent.worldAABB =
          calculateColliderAABB(*rb, compCollider, transform, entityScale);
    } else {
      // Fallback to model-based AABB
    ...
    if (m_bvh.isBuilt() && m_bvhItems.size() < m_lastEntityCount * 1.2) {
      bvComponent.bvhNodeIndex =
          m_bvh.insertLeaf(entity, bvComponent.worldAABB);
    } else {
  ...
  for (auto trackedEntity : m_trackedEntities) {
    if (m_currentFrameEntities.find(trackedEntity) ==
        m_currentFrameEntities.end()) {
  ...
  if (m_needsFullRebuild || !m_bvh.isBuilt()) {
    ...
    auto rebuildTime = std::chrono::duration_cast<std::chrono::microseconds>(
