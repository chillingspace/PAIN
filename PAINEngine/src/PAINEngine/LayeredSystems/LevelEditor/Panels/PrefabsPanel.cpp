	...
	namespace Editor {
		namespace Panel {
			...
			bool PrefabPanel::enterEditMode(const Assets::GUID& prefabGUID) {
				...
				PN_CORE_INFO("[PrefabEditMode] Entering edit mode for prefab: {}",
							 prefabGUID.ToString());
				...
				if (!prefabAssetOpt.has_value()) {
					PN_CORE_ERROR("[PrefabEditMode] Prefab asset not found: {}",
								  prefabGUID.ToString());
					return false;
				}
				...
				currentPrefabName = prefabAsset->prefabName.empty()
										? prefabAsset->main_relative_path.stem().string()
										: prefabAsset->prefabName;
				...
				PN_CORE_INFO("[PrefabEditMode] Created edit registry: {} (ID: {})",
							 registryName, editRegistryID);
				...
				editRootEntity =
					prefabService->loadPrefabForEditing(prefabGUID, editRegistryID);
				if (editRootEntity == entt::null) {
					PN_CORE_ERROR(
						"[PrefabEditMode] Failed to instantiate prefab in edit registry");
				...
				PN_CORE_INFO("[PrefabEditMode] Removed {} PrefabInstance components",
							 entitiesToClean.size());
				...
				PN_CORE_INFO(
					"[PrefabEditMode] Successfully entered edit mode. Root entity: {}",
					static_cast<uint32_t>(editRootEntity));
			...
			bool PrefabPanel::saveCurrentPrefab() {
				...
				prefabService->collectHierarchy(editRootEntity, hierarchyEntities,
												editRegistryID);
				PN_CORE_INFO("[PrefabEditMode] Collected {} entities to serialize",
							 hierarchyEntities.size());

				// Create prefab
				Prefab::PrefabAsset updatedPrefab;
				updatedPrefab.rootEntityGUID =
					editRegistry.get<Entity::GUID>(editRootEntity).guid;
				...
				for (auto e : hierarchyEntities) {
					updatedPrefab.entities.push_back(
						prefabService->serializeEntity(e, editRegistryID));
				}
				...
				std::string virt_path_to_prefab = path_service->aliasCombineRelative(
					...
					prefab_folder.string() + "/" + updatedPrefab.name + prefab_ext);

				// Save prefab to file
				prefabService->savePrefabToFile(updatedPrefab, virt_path_to_prefab,
												editRegistryID);
				...
				prefabService->updateAllInstances(currentEditingPrefabGUID,
												  ECS::MAIN_REGISTRY_ID, true);
				prefabService->updateAllInstances(currentEditingPrefabGUID, editRegistryID,
												  true);
				hasUnsavedChanges = false;
				PN_CORE_INFO("[PrefabEditMode] Successfully saved prefab: {}",
							 updatedPrefab.name);
				return true;
			}
			...
			bool PrefabPanel::revertChanges() {
				...
				PN_CORE_INFO("[PrefabEditMode] Reverting changes to prefab: {}",
							 currentPrefabName);
				// Baic approach
			...
			void PrefabPanel::showUnsavedChangesDialog() {
				...
				if (ImGui::BeginPopupModal("Unsaved Changes##PrefabEditor", nullptr,
					...
					ImGui::TextWrapped(
						"The prefab has unsaved changes. What would you like to do?");
				...
				if (showUnsavedWarning &&
					!ImGui::IsPopupOpen("Unsaved Changes##PrefabEditor")) {
			...
			void PrefabPanel::renderInstanceTools() {
				...
				auto prefab_instances =
					services->get<Prefab::Service>()->getInstancesOfPrefab(
						currentEditingPrefabGUID);
