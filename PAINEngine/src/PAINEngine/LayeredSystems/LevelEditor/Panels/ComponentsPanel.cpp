...
void ComponentsPanel::onAttach() {
  ...
  // ---- CompoundCollider ----
  registerCompUIFunc<
      PAIN::CompoundCollider>("CompoundCollider", [](ComponentsPanel &,
                                                     PAIN::CompoundCollider
                                                         &cc) {
    bool changed = false;

    // Main toggle
    if (ImGui::Checkbox("Use Compound Collider", &cc.useCompoundCollider)) {
      changed = true;
    }
    ImGui::TextDisabled(
        "Enable to use custom multi-shape collision instead of default AABB");

    ImGui::Spacing();
    ImGui::Separator();

    if (!cc.useCompoundCollider) {
      ImGui::BeginDisabled();
    }

    // Shape list header
    ImGui::Text("Collider Shapes (%zu)", cc.shapes.size());

    // Add shape buttons
    if (ImGui::Button("+ Add Box")) {
      cc.shapes.push_back(ColliderShape::CreateBox(glm::vec3(0.5f)));
      changed = true;
    }
    ImGui::SameLine();
    if (ImGui::Button("+ Add Sphere")) {
      cc.shapes.push_back(ColliderShape::CreateSphere(0.5f));
      changed = true;
    }
    ImGui::SameLine();
    if (ImGui::Button("+ Add Capsule")) {
      cc.shapes.push_back(ColliderShape::CreateCapsule(0.25f, 0.5f));
      changed = true;
    }
    ...
    // Render each shape
    int removeIndex = -1;
    for (size_t i = 0; i < cc.shapes.size(); ++i) {
      auto &shape = cc.shapes[i];
      ImGui::PushID(static_cast<int>(i));

      // Shape type name
      const char *typeNames[] = {"Box", "Sphere", "Capsule"};
      const char *typeName = typeNames[static_cast<int>(shape.type)];

      if (ImGui::CollapsingHeader(
              (std::string("[") + std::to_string(i) + "] " + typeName).c_str(),
        ...
        if (ImGui::Combo("Type", &typeIdx, typeNames,
                         IM_ARRAYSIZE(typeNames))) {
          shape.type = static_cast<ColliderShapeType>(typeIdx);
          changed = true;
        }

        // Common: offset
        if (ImGui::DragFloat3("Offset", &shape.offset.x, 0.05f)) {
          changed = true;
        }
        ...
        case ColliderShapeType::Box:
          if (ImGui::DragFloat3("Half Extents", &shape.boxHalfExtents.x, 0.05f,
                                0.01f, 100.0f)) {
            changed = true;
          }
          break;
        case ColliderShapeType::Sphere:
          if (ImGui::DragFloat("Radius", &shape.sphereRadius, 0.05f, 0.01f,
                               100.0f)) {
            changed = true;
          }
          break;
        case ColliderShapeType::Capsule:
          if (ImGui::DragFloat("Radius", &shape.capsuleRadius, 0.05f, 0.01f,
                               100.0f)) {
            changed = true;
          }
          if (ImGui::DragFloat("Half Height", &shape.capsuleHalfHeight, 0.05f,
                               0.01f, 100.0f)) {
            changed = true;
          }
          break;
    ...
    if (removeIndex >= 0 && removeIndex < static_cast<int>(cc.shapes.size())) {
      cc.shapes.erase(cc.shapes.begin() + removeIndex);
      changed = true;
    }
